    ...
    def train(self):
        ...
        for epoch in range(self.opt.continue_train_epoch, self.opt.train_epoch):
            ...
            if self.opt.rank == 0:
                ...
                self.writer.add_scalar(f'liver_hds/train_epoch', liver_hds.avg, epoch)
                ...
                self.writer.add_scalar(f'tumor_hds/train_epoch', tumor_hds.avg, epoch)
        ...
        with torch.no_grad():
            ...
            for iter_test, (image, mask) in tqdm(enumerate(self.test_loader), total=len(self.test_loader)):
                ...
                if self.opt.rank == 0:
                    ...
                    self.writer.add_scalar(f'liver_mious/iter_test', liver_miou, epoch)
                    self.writer.add_scalar(f'liver_hds/iter_test', liver_hd, epoch)
                    ...
                    self.writer.add_scalar(f'tumor_mious/iter_test', tumor_miou, epoch)
                    self.writer.add_scalar(f'tumor_hds/iter_test', tumor_hd, epoch)
